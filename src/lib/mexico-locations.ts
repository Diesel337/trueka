// Generated from geo-latam 0.1.0, based on INEGI Cat?logo ?nico de Claves Geoestad?sticas.
// Keep this file data-only; UI helpers live below the generated catalog.
...
export type MexicoLocationZone = {
  value: string;
  label: string;
  state: string;
...
] as const satisfies readonly MexicoState[];

export const mexicoMunicipalitiesByState: Record<string, readonly MexicoMunicipality[]> = {
  "Aguascalientes": [
    {
      "code": "001",
      "name": "Aguascalientes"
...
export const guadalajaraMetroLocationValue = "zmg";

export const mexicoLocationZones = [
  {
...
] as const satisfies readonly MexicoLocationZone[];

const municipalityAliasesByState: Record<string, Record<string, string>> = {
  Jalisco: {
...
export function getMexicoStateDisplayName(stateName: string) {
  return stateDisplayNames[stateName] ?? stateName;
}

export function getMunicipalitiesForState(stateName: string) {
  return mexicoMunicipalitiesByState[stateName] ?? [];
}

export function getZonesForState(stateName: string) {
  return mexicoLocationZones.filter((zone) => zone.state === stateName);
}

export function isValidMexicoState(stateName: string) {
  return mexicoStates.some((state) => state.name === stateName);
}

export function isValidStateMunicipality(stateName: string, municipalityName: string) {
  const canonicalMunicipality = getCanonicalMunicipalityName(stateName, municipalityName);

  return getMunicipalitiesForState(stateName).some(
    (municipality) => municipality.name === canonicalMunicipality,
  );
}

export function getCanonicalMunicipalityName(stateName: string, municipalityName: string) {
  const aliases = municipalityAliasesByState[stateName] ?? {};
  const normalizedMunicipality = normalizeLocationName(municipalityName);

  return aliases[normalizedMunicipality] ?? municipalityName;
}

export function isLocationInFilter(
  input: { state: string; municipality: string },
  filter: { state?: string; municipality?: string },
) {
  ...
  const canonicalFilterMunicipality = state
    ? getCanonicalMunicipalityName(state, municipality)
    : municipality;
  const zone = mexicoLocationZones.find(
    (entry) => entry.value === municipality && (!state || entry.state === state),
  );

  if (zone) {
    const zoneMunicipalities: readonly string[] = zone.municipalities;

    return input.state === zone.state
      && zoneMunicipalities.includes(getCanonicalMunicipalityName(input.state, input.municipality));
  }

  return getCanonicalMunicipalityName(input.state, input.municipality) === canonicalFilterMunicipality;
}

export function getMunicipalitiesForFilter(stateName?: string, municipality?: string) {
  if (!municipality) {
    return [];
  }

  const zone = mexicoLocationZones.find(
    (entry) => entry.value === municipality && (!stateName || entry.state === stateName),
  );

  return zone ? [...zone.municipalities] : [stateName ? getCanonicalMunicipalityName(stateName, municipality) : municipality];
}

export function getStateForLocationFilter(municipality?: string) {
  if (!municipality) {
    return undefined;
  }

  return mexicoLocationZones.find((zone) => zone.value === municipality)?.state;
}
